...
Ext.define('Extensible.calendar.form.EventWindow', {
    ...
    onSave: function() {
    	 ...
    	 Ext.Ajax.request({
                          url : 'service/calendar.php',
                          method:'post',
                          params: {
                          	action:1,
                         // mod : 'addCal',
                         //user : MyApp.global.Vars.user,
                          title : this.titleField.getValue(),
                         dateStart :  this.dateRangeField.getValue()[0],
                         dateEnd :  this.dateRangeField.getValue()[1],
                         allDay : this.dateRangeField.getValue()[2],
                        //  color : this.calendarField.getValue(),
                          // : Ext.get('who').getValue()
                                                                }
                  }); 
     
        if ((!form.isDirty() && !me.activeRecord.phantom) || (!me.allowDefaultAdd && me.activeRecord.phantom)) {
   me.onCancel();
   return;
}        if (!form.isValid()) {
            return;
